...
// GET /api/admin/para29/overall-stats — total recordings per user (all time)
app.get('/api/admin/para29/overall-stats', adminAuth, async (req, res) => {
  try {
    const stats = await Para29Recording.aggregate([
      {
        $group: {
      ...
      { $sort: { totalCount: -1, recorderName: 1 } }
    ]);

    const grandTotal = stats.reduce((sum, s) => sum + s.totalCount, 0);
    ...
    console.error('Error fetching para29 overall stats:', err);
