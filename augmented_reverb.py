...
augmented_folder = "augmented_reverb"   # Reverberated files
os.makedirs(augmented_folder, exist_ok=True)

# CSV path
csv_path = "recorded_ayats_wav_augmented.csv"

print("=== Starting Data Augmentation (Reverberation) ===\n")
...
for idx, row in df.iterrows():
    ...
    try:
        ...
        output_name = filename.replace('.wav', '_reverb.wav')
        output_path = os.path.join(augmented_folder, output_name)
        
        # FFmpeg command to add reverberation effect
        # Using "aecho" filter: aecho=in_gain:out_gain:delays:decays
        # Example: mild reverb (0.8 gain, 0.9 decay)
        cmd = [
            'ffmpeg', '-i', input_path,
            '-filter:a', 'aecho=0.8:0.9:1000:0.3',
            output_path,
            '-y',
            '-loglevel', 'error'
        ]
        ...
        df.at[idx, 'augmented_audio_reverb'] = output_path
...
# Save new CSV
output_csv = "recorded_ayats_wav_reverb.csv"
df.to_csv(output_csv, index=False)

print("\n" + "="*50)
print("=== AUGMENTATION COMPLETE (REVERB) ===")
print("="*50)
...
print("   - 'augmented_audio_reverb': Path to reverberated audio")
